  ...
  stage('OWASP ZAP - DAST') {
    steps {
        withKubeConfig([credentialsId: 'kubeconfig']) {
            ...
            sh 'docker exec zap_dast zap-cli quick-scan --self-contained --start-options "-config api.disablekey=true" http://your-target-url'
            sh 'docker exec zap_dast zap-cli report -o /zap/wrk/zap_report.html -f html'
